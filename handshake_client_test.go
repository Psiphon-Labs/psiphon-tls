...
func testResumption(t *testing.T, version uint16) {
	...
	getTicket := func() []byte {
		return fromClientSessionState(clientConfig.ClientSessionCache.(*lruSessionCache).q.Front().Value.(*lruSessionCacheEntry).state).ticket
	}
	deleteTicket := func() {
	...
	corruptTicket := func() {
		fromClientSessionState(clientConfig.ClientSessionCache.(*lruSessionCache).q.Front().Value.(*lruSessionCacheEntry).state).session.secret[0] ^= 0xff
	}
...
func TestKeyLogTLS12(t *testing.T) {
	...
	go func() {
		...
		if err := Server(s, &ExtendedTLSConfig{TLSConfig: serverConfig}).Handshake(); err != nil {
	...
	if err := Client(c, &ExtendedTLSConfig{TLSConfig: clientConfig}).Handshake(); err != nil {
...
func TestServerSelectingUnconfiguredApplicationProtocol(t *testing.T) {
	...
	go func() {
		client := Client(c, &ExtendedTLSConfig{
			TLSConfig: &Config{
				ServerName:   "foo",
...
func TestHostnameInSNI(t *testing.T) {
	for _, tt := range hostnameInSNITests {
		...
		go func(host string) {
			Client(c, &ExtendedTLSConfig{TLSConfig: &Config{ServerName: host, InsecureSkipVerify: true}}).Handshake()
		}(tt.in)
...
func testVerifyPeerCertificate(t *testing.T, version uint16) {
	...
	for i, test := range tests {
		...
		go func() {
			...
			err = Server(s, &ExtendedTLSConfig{TLSConfig: config}).Handshake()
			s.Close()
			done <- err
		}()
		...
		clientErr := Client(c, &ExtendedTLSConfig{TLSConfig: config}).Handshake()
		c.Close()
		serverErr := <-done
...
func TestFailedWrite(t *testing.T) {
	...
	for _, breakAfter := range []int{0, 1} {
		...
		err := Client(brokenC, &ExtendedTLSConfig{TLSConfig: testConfig}).Handshake()
...
func testBuffering(t *testing.T, version uint16) {
	...
	go func() {
		config := testConfig.Clone()
		config.MaxVersion = version
		Server(serverWCC, &ExtendedTLSConfig{TLSConfig: config}).Handshake()
		serverWCC.Close()
		done <- true
	}()

	err := Client(clientWCC, &ExtendedTLSConfig{TLSConfig: testConfig}).Handshake()
	if err != nil {
		t.Fatal(err)
	}
	clientWCC.Close()
...
func TestAlertFlushing(t *testing.T) {
	...
	go func() {
		Server(serverWCC, &ExtendedTLSConfig{TLSConfig: serverConfig}).Handshake()
		serverWCC.Close()
		done <- true
	}()
...
func TestHandshakeRace(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			server := Server(s, &ExtendedTLSConfig{TLSConfig: testConfig})
		...
		client := Client(c, &ExtendedTLSConfig{TLSConfig: testConfig})
		go func() {
			<-startWrite
			var request [1]byte
...
func TestCloseClientConnectionOnIdleServer(t *testing.T) {
	clientConn, serverConn := localPipe(t)
	client := Client(clientConn, &ExtendedTLSConfig{TLSConfig: testConfig.Clone()})
	go func() {
		var b [1]byte
		serverConn.Read(b[:])
		client.Close()
